    ...
    try {
        ...
        if (payload) {
            ...
            if (existingBoundUser && existingBoundUser._id.toString() !== user._id.toString()) {
                return ctx.reply(`⚠️ Link Rejected: This Telegram account is already linked to another user (${existingBoundUser.email}).`);
            }
            ...
            return ctx.reply(`🎉 <b>Account Linked Successfully!</b>\n\nWelcome <b>${user.name}</b> (${user.role.toUpperCase()}). Your Telegram account is connected to Alpha Cut Executive ERP.`, { parse_mode: 'HTML', ...menuKb });
        }
        ...
        if (existingUser) {
            ...
            return ctx.reply(`🎬 <b>Welcome back, ${existingUser.name}!</b>\n\nUse your role-based menu below to manage projects, review work, or launch the Mini App.`, { parse_mode: 'HTML', ...menuKb });
        }
        ctx.reply(`🎬 <b>Welcome to Alpha Cut Control Center</b>\n\n` +
            ...
            `2. Enter your 6-digit code using <code>/link 123456</code>`, { parse_mode: 'HTML', ...getUnlinkedMenuKeyboard() });
    }
    catch (err) {
