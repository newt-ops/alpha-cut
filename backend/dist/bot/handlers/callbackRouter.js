...
export const handleCallbackQuery = async (ctx) => {
    ...
    try {
        ...
        if (data === 'menu:main') {
            ...
            return safeEditMessageText(ctx, user?.role === 'admin' ? '⚡ <b>Alpha Cut Admin ERP Menu</b>' : '🎬 <b>Alpha Cut Client Control Menu</b>', { parse_mode: 'HTML', ...menuKb });
        }
        if (data === 'menu:support') {
            return safeEditMessageText(ctx, `💬 <b>ALPHA CUT SUPPORT & INQUIRIES</b>\n\n` +
                ...
                `• <b>Web Platform:</b> ${CLIENT_URL}`, { parse_mode: 'HTML', ...getSupportKeyboard() });
        }
