...
export const getClientStylesPaginationKeyboard = (pageIdx, totalPages) => {
    const navRow = [];
    if (pageIdx > 0)
        navRow.push(Markup.button.callback('◀️ Prev Style', `menu:styles:${pageIdx - 1}`));
    if (pageIdx < totalPages - 1)
        navRow.push(Markup.button.callback('Next Style ▶️', `menu:styles:${pageIdx + 1}`));
    return Markup.inlineKeyboard([
