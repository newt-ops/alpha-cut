...
export const markDelivered = async (req, res, next) => {
    try {
        ...
        const project = await lifecycleService.markDelivered(id, deliverableUrl, req.user._id);
...
export const updatePackageConfig = async (req, res, next) => {
    try {
        ...
        const config = await PackageConfig.findOneAndUpdate({ tier, length, currency }, { priceMin, priceMax, features }, { upsert: true, new: true });
