...
import { bot, updateContractTelegramStatusCard, sendProposalNotificationTelegram, sendDeliveryNotificationTelegram, sendRevisionNotificationTelegram, } from './telegram.service.js';
...
export const acceptProposal = async (projectId, clientId) => {
    ...
    if (project.status !== 'proposal_sent')
        throw new Error(`Cannot accept proposal in status: ${project.status}`);
...
export const declineProposal = async (projectId, clientId) => {
    ...
    if (!project)
        throw new Error('Project not found or access denied.');
    if (project.status !== 'proposal_sent')
        throw new Error(`Cannot decline proposal in status: ${project.status}`);
...
export const approveDelivery = async (projectId, clientId) => {
    ...
    if (project.status !== 'delivered')
        throw new Error(`Cannot approve delivery in status: ${project.status}`);
...
export const requestRevision = async (projectId, clientId, revisionNotes) => {
    ...
    if (!project)
        throw new Error('Project not found or access denied.');
    if (project.status !== 'delivered')
        throw new Error(`Revisions can only be requested for delivered projects.`);
    if (!revisionNotes || !revisionNotes.trim())
        throw new Error('Please provide specific revision notes.');
...
export const submitRating = async (projectId, clientId, stars, review) => {
    ...
    if (project.rated)
        throw new Error('Project has already been rated.');
    if (project.status === 'delivered') {
        project.status = 'completed';
        project.completedAt = new Date();
    }
...
export const declineContract = async (contractId, clientId) => {
    ...
    if (!contract)
        throw new Error('Contract not found or access denied.');
    if (contract.status !== 'proposed')
        throw new Error(`Cannot decline contract in status: ${contract.status}`);
...
export const addDeliverable = async (contractId, adminId, data) => {
    ...
    if (contract.status !== 'active')
        throw new Error(`Cannot add deliverables to contract in status: ${contract.status}`);
...
export const completeContract = async (contractId, adminId) => {
    ...
    if (!contract)
        throw new Error('Contract not found.');
    if (contract.status !== 'active')
        throw new Error(`Cannot complete contract in status: ${contract.status}`);
...
export const submitContractRating = async (contractId, clientId, stars, review) => {
    ...
    if (contract.status !== 'completed')
        throw new Error('Rating is only allowed for completed retainer contracts.');
    if (contract.rated)
        throw new Error('Contract has already been rated.');
    const client = await User.findById(clientId);
    const rating = await Rating.create({
