...
export const handleStatusCommand = async (ctx) => {
  try {
    ...
    if (!user) return ctx.reply('⚠️ Please link your account first to check status.', getUnlinkedMenuKeyboard());
    ...
    if (contract) {
      const approvedCount = contract.deliverables?.filter((d) => d.status === 'approved').length || 0;
      const totalCount = contract.totalVideosPlanned || contract.deliverables?.length || 0;
    ...
    const actionId = project ? project._id : contract._id;
