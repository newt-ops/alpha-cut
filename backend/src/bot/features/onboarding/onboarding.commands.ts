...
export const handleStartCommand = async (ctx: Context): Promise<any> => {
  try {
    ...
    return ctx.reply(
      `🎬 <b>Welcome to Alpha Cut Bot!</b>\n\n` +
        ...
        `3. Send <code>/link 123456</code> here`,
...
export const handleHelpCommand = async (ctx: Context): Promise<any> => {
  try {
    return ctx.reply(
      `💡 <b>ALPHA CUT TELEGRAM BOT COMMANDS</b>\n\n` +
...
export const handleLinkCommand = async (ctx: Context): Promise<any> => {
  try {
    const message = ctx.message as any;
    const parts = (message?.text || '').split(' ');
    const code = parts[1];
...
export const handleUnlinkCommand = async (ctx: Context): Promise<any> => {
  try {
    const chatId = ctx.chat?.id.toString();
    if (!chatId) return;
