...
export const handleCallbackQuery = async (ctx) => {
  ...
  try {
    ...
    if (data === 'menu:work' || data === 'menu:projects') {
      ...
      // Security check: Query strictly for user._id
    ...
    // Toggle In-Place Message Brief Details (Security check: client ownership)
    if (data.startsWith('toggle_details_proj_')) {
      ...
      if (user?.role !== 'admin' && project.clientId.toString() !== user?._id.toString()) {
        return; // Security guard: block unauthorized access to other projects
      }

      const isShowingBrief = ctx.callbackQuery.message.text.includes('Reference Brief Notes:');
      let updatedText = '';
