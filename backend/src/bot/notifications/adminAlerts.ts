...
export const getAdminChatIds = async (): Promise<string[]> => {
  try {
...
export const sendTelegramNotification = async (bot: Telegraf | null, chatId: string, text: string): Promise<void> => {
...
export const sendRevisionNotificationTelegram = async (bot: Telegraf | null, project: IProject): Promise<void> => {
  if (!bot) return;
