...
export const sendProposalNotificationTelegram = async (bot: Telegraf | null, project: IProject, clientChatId: string): Promise<void> => {
...
export const sendDeliveryNotificationTelegram = async (bot: Telegraf | null, project: IProject, clientChatId: string): Promise<void> => {
  if (!bot || !clientChatId) return;
