...
export const getAllClients = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
...
export const createProposal = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
...
export const getAllProjects = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    ...
    if (search) {
      query.$or = [
        ...
        { editingStyle: { $regex: search as string, $options: 'i' } },
      ];
    }
...
export const updateProjectAdminNotes = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    const { id } = req.params;
...
export const updateClientAdminNotes = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    const { id } = req.params;
    const { adminNotes } = req.body;
...
export const getStats = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    ...
    const toETB = (price: number, currency: string = 'ETB') => (currency === 'USD' ? Math.round(price * exchangeRate) : price);
    const toUSD = (price: number, currency: string = 'ETB') => (currency === 'ETB' ? Number((price / exchangeRate).toFixed(2)) : price);
    ...
    const revenueByTier: Record<string, Record<string, number>> = {
      basic: { USD: 0, ETB: 0 },
    ...
    const revenueByStyleMap: Record<string, { USD: number; ETB: number }> = {};
    billableProjects.forEach((p) => {
    ...
    const clientRevenueMap: Record<string, { name: string; email: string; totalETB: number; totalUSD: number; count: number }> = {};
    billableProjects.forEach((p) => {
    ...
    const monthlyTrendMap: Record<string, { label: string; USD: number; ETB: number }> = {};
...
export const updatePackageConfig = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
...
export const getAllPackageConfigs = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
...
export const getLiveExchangeRate = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    let usdToEtbRate = 128.5;
