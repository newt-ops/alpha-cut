...
export const getChapaStatus = async (req: Request, res: Response): Promise<void> => {
  res.status(200).json({
    success: true,
...
export const toggleChapaTestMode = async (req: Request, res: Response): Promise<void> => {
  ...
  if (secretKey && (secretKey as string).trim().length > 0) {
    process.env.CHAPA_SECRET_KEY = (secretKey as string).trim();
  }
...
export const initializeChapaPayment = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    if (!isTestModeActive) {
...
export const confirmProjectPayment = async (txRef: string, chapaPayload: any = {}): Promise<{ itemType: string | undefined; itemId: any; alreadyConfirmed: boolean }> => {
  let payment = await Payment.findOne({ txRef });
  let itemType: string | undefined = payment?.subjectType;
  let itemId: any = payment?.subjectId;
...
export const verifyChapaPayment = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    const { txRef } = req.params;
    const refString = txRef as string;
...
export const handleChapaWebhook = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
