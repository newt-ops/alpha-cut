...
router.post('/webapp/link-code', async (req, res, next) => {
  try {
    ...
    // 1. FAST CHECK: Is this chatId ALREADY bound to a user profile in MongoDB?
    if (chatId) {
    ...
    // 2. Query PendingLink by code (allow recent codes)
    ...
    // If not found as unused, check if recently created for matching
    if (!pending) {
