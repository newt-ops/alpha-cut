...
export const RatingsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        setLoading(true);
        const res = await customFetch('/api/ratings?page=1&limit=6');
        if (res.success) {
  ...
  const handleLoadMore = async () => {
    ...
    try {
      setLoadingMore(true);
      const res = await customFetch(`/api/ratings?page=${nextPage}&limit=6`);
      if (res.success) {
        ...
        <>
          ...
          {currentPage < totalPages && (
            <div style={{ textAlign: 'center', marginTop: '40px' }}>
              ...
              >
                Load More Reviews ({ratingsList.length} of {totalReviews})
              </Button>
            </div>
          )}
        </>
